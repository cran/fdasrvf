...
double Spline::ValSpline(const double *coefs, const double *breaks, int N, double t)
{
	...
	nn = N - 1;
	i = 0;
	while (t - (breaks[i] - breaks[0]) >= -std::numeric_limits<double>::epsilon())
		i++;
	i--;
	i = (i < 0) ? 0 : i;
	i = (i > nn - 1) ? nn - 1 : i;
...
double Spline::ValFirstDeri(const double *dericoefs, int N, double h, double t)
{
	int i, nn;
	nn = N - 1;
...
double Spline::ValSecondDeri(const double *dericoefs, int N, double h, double t)
{
	int i, nn;
	nn = N - 1;
	i = static_cast<int> (t / h);
